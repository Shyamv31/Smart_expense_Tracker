...
class GeminiService {
  static const _apiKey = 'AIzaSyBQEzZnn9agtL2emESyL0fboNE3WGXBkm8';
  ...
  // Scan receipt image
    Future<Map<String, dynamic>> scanReceipt(Uint8List imageBytes, [String mimeType = 'image/jpeg']) async {
    try {
      final prompt = '''
...
''';
      ...
      print('DEBUG parsed: title=$title amount=$amountStr category=$category note=$note');
      return {
        'title': title,
  ...
  // AI spending advice
  Future<String> getSpendingAdvice(Map<String, double> categoryTotals,
      double totalSpent, double budget) async {
    try {
      final prompt = '''
...
''';
  ...
  Future<String> getPrediction(
      List<Map<String, dynamic>> recentExpenses) async {
    try {
...
''';
  ...
  String _guessCategory(String title, String note) {
    ...
    if (text.contains(RegExp(
      ...
      return 'Food';
    if (text.contains(RegExp(
        r'bus|train|metro|cab|uber|ola|petrol|fuel|transport|auto|taxi')))
      return 'Transport';
    if (text.contains(RegExp(
        r'grocery|vegetable|fruit|supermarket|bigbasket|blinkit|milk|rice')))
      return 'Groceries';
    if (text.contains(RegExp(
        r'shop|mall|cloth|amazon|flipkart|fashion|dress|shirt')))
      return 'Shopping';
    if (text.contains(RegExp(
        r'hospital|medical|pharmacy|doctor|medicine|health|clinic')))
      return 'Health';
    if (text.contains(
        RegExp(r'movie|cinema|game|entertainment|netflix|sport')))
